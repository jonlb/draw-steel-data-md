#!/usr/bin/env python3
"""
Parse organization markdown files from Rules/Cultures/Organization directory into structured JSON.
"""
...
def parse_organization_file(file_path):
    """Parse a single organization markdown file."""
    ...
    # Remove the heading (##### Organization Name)
...
def main():
    ...
    organizations_dir = project_root / "Rules" / "Cultures" / "Organization"
    output_file = project_root / "data" / "organizations.json"
    
    print(f"Parsing organizations from {organizations_dir}...")
    
    organizations = []
    
    # Process each markdown file in the directory
    for file_path in sorted(organizations_dir.glob("*.md")):
        print(f"Parsing {file_path.name}...")
        organization = parse_organization_file(file_path)
        organizations.append(organization)
    
    # Write to JSON file
    print(f"Writing {len(organizations)} organizations to {output_file}...")
    output_file.parent.mkdir(parents=True, exist_ok=True)
    
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(organizations, f, indent=2, ensure_ascii=False)
    
    print(f"Done! Organizations saved to {output_file}")
